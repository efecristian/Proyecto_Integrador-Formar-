...
function validarRegistro($datos){
  ...
  if ($datos) {
    ...
    if ($datos["contrasenia"] != $datos["confircontra"]) {
      $errores[] = "Las contraseñas no coinciden";
    }
    if ($datos["telefono"] == 0) {
      $errores[] = "El telefono se encuentra vacio";
    }
    if ($datos["telefono"] < 8) {
      $errores[] = "Ingrese un telefono valido";
    }
    if ($datos["nacionalidad"]=="Selecciona un Pais") {
      $errores[] = "Por favor ingrese su nacionalidad";
    }
    // if ($datos["genero"]==null) {
    //   $errores[] = "Ingresa tu sexo";
    // }
    // En esta seccion utilizo la  variable FILES para validar que la imagen que cargó el usuario haya llegado de forma correcta y tenga la extension correspondiente.
    if ($_FILES != null){
      if ($_FILES["avatar"]["error"]!=0){
        $errores["avatar"] = "La imagen no se ha cargado";
      }
      $nombimg = $_FILES["avatar"]["name"];
      $ext = pathinfo($nombimg, PATHINFO_EXTENSION);
      if ($ext != "jpg" && $ext != "jpeg" && $ext != "png" && $ext != "PNG") {
        $errores["avatar"] = "La extension del archivo es incorrecta";
      }
    }
    return $errores;
  }
...
function armarUsuario($datos, $imagen){
  ...
  // Crea un array usuario  que en cada posicion recibira lo que el usuario ingreso en el formulario salvo en las posiciones de contraseña (va a recibir la contraseña previamente hasheada) y el avatar (recibira el nombre final de la imagen con el que fue guardado en el servidor.)
  $usuario = [
    "nombre" => $datos["nombre"],
